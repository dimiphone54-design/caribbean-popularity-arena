...
function FygaroReturnContent() {
  const searchParams = useSearchParams();
  const reference = searchParams.get("reference") ?? searchParams.get("Reference");
  const customReference =
    searchParams.get("customReference") ?? searchParams.get("custom_reference");
  ...
  useEffect(() => {
    if (!reference && !customReference) return;
    ...
    if (product === "member-sign-in") {
      ...
      setMessage("Member gift confirmed · profile saved · welcome panel unlocked.");
      return;
    }
  ...
  }, [customReference, product, reference]);
    ...
    <main className="mx-auto flex min-h-[70vh] max-w-xl flex-col justify-center px-6 py-16 text-center">
      ...
      {hasReturnParams ? (
        ...
        <p className="mt-6 text-sm leading-6 text-[#9aa8c6]">
          No payment reference was found. If you just completed a gift, wait a moment and refresh — or return to the
          arena and try again.
        </p>
      )}
...
export default function FygaroReturnPage() {
  return (
    <Suspense
      fallback={
