...
const CfaPopularityArenaWorldMapModal = dynamic(
  () =>
    import("@/components/cfa-popularity-arena-world-map-modal").then(
      (mod) => mod.CfaPopularityArenaWorldMapModal
    ),
  { ssr: false, loading: () => null }
);
import {
  PopularityArenaRecordsModal,
...
export function LiveArenaExperience() {
  ...
  const openPopularityRecordsPanel = (tab: PopularityRecordsTab) => {
    setPopularityRecordsTab(tab);
    setOpenPopularityRecords(true);
  };
  ...
  useEffect(() => {
    const openFromHash = () => {
      ...
      if (hash === "sports" || hash === "sports-tab") {
        setPopularityRecordsTab("sports");
        setOpenPopularityRecords(true);
        return;
      }
      if (hash === "world-map" || hash === "freedom-map") {
        setOpenPopularityWorldMap(true);
        return;
      }
      if (hash === "popularity" || hash === "popularity-records" || hash === "popularity-hall") {
        setPopularityRecordsTab("matches");
        setOpenPopularityRecords(true);
      }
    };
  ...
  return (
    ...
    <section id="home" className="arena-2030 arena-2030--hero-dubai relative isolate min-h-screen overflow-hidden text-[#f7efe0]">
      ...
      <nav className="a2030-nav fixed inset-x-0 top-0 z-50 flex h-[58px] min-w-0 items-center justify-between gap-2 overflow-hidden px-4 sm:px-7">
        <div className="a2030-nav-brand-group cfa-nav-quantum-lockup cfa-nav-quantum-lockup--holo min-w-0 shrink">
          <CfaNavFreedomElite2050Pill onOpenWorldMap={() => setOpenPopularityWorldMap(true)} />
          <CfaNavPopularityArenaPill />
      ...
      <CfaNavSportsVideoPanel onOpenRecords={() => openPopularityRecordsPanel("sports")} />

      <div className="a2030-island-bar fixed inset-x-0 z-40 flex overflow-hidden">
      ...
      {openPopularityWorldMap ? (
        <CfaPopularityArenaWorldMapModal
          open
          onClose={() => setOpenPopularityWorldMap(false)}
        />
      ) : null}

      <PopularityArenaRecordsModal
        open={openPopularityRecords}
        initialTab={popularityRecordsTab}
        onClose={() => setOpenPopularityRecords(false)}
      />
