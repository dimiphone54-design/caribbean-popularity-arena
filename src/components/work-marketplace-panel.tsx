...
export function WorkMarketplacePanel() {
  const [jobs, setJobs] = useState<Job[]>([]);
  const [profiles, setProfiles] = useState<Profile[]>([]);

  const [job, setJob] = useState({ job_title: "", company: "", work_mode: "Remote", description: "", contact_url: "" });
  const [prof, setProf] = useState({ display_name: "", skill: "", location: "", description: "", contact_url: "", availability: "Available Now" });
  ...
  async function loadJobs() {
    try {
      const r = await fetch("/api/work/jobs");
      setJobs(await r.json());
    } catch { /* ignore */ }
  }
  async function loadProfiles() {
    try {
      const r = await fetch("/api/work/profiles");
      setProfiles(await r.json());
    } catch { /* ignore */ }
  }
  useEffect(() => { loadJobs(); loadProfiles(); }, []);
  ...
  async function postJob() {
    ...
    try {
      ...
      if (d.ok) { setJobMsg("Posted!"); setJob({ job_title: "", company: "", work_mode: "Remote", description: "", contact_url: "" }); loadJobs(); }
  ...
  async function postProfile() {
    ...
    try {
      ...
      const d = await r.json();
      if (d.ok) { setProfMsg("Posted!"); setProf({ display_name: "", skill: "", location: "", description: "", contact_url: "", availability: "Available Now" }); loadProfiles(); }
  ...
  return (
    <section className="w-full" aria-label="Work Marketplace">
      <div className="mt-6 overflow-hidden rounded-2xl border border-cyan-500/20 bg-gradient-to-br from-cyan-900/20 via-[#030712]/90 to-emerald-900/20 p-4 sm:p-6">
        <div className="flex items-center gap-2">
          ...
          <span className="ml-auto rounded-full border border-emerald-500/30 bg-emerald-500/10 px-3 py-1 text-[10px] font-black uppercase tracking-wider text-emerald-400">Free · Direct</span>
        </div>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          ...
          <div className="rounded-xl border border-emerald-500/20 bg-black/40 p-4">
            ...
            <div className="mt-4 space-y-2">
              {jobs.length === 0 && <p className="text-[10px] text-white/30">No jobs yet — be the first to post.</p>}
              {jobs.map((j) => (
                <div key={j.id} className="rounded-lg border border-white/5 bg-white/5 px-3 py-2">
                  ...
                  <p className="text-[9px] text-white/40">{[j.company, j.work_mode].filter(Boolean).join(" · ")}</p>
          ...
          <div className="rounded-xl border border-cyan-500/20 bg-black/40 p-4">
            ...
            <div className="mt-4 space-y-2">
              {profiles.length === 0 && <p className="text-[10px] text-white/30">No profiles yet — be the first to post.</p>}
              {profiles.map((p) => (
                <div key={p.id} className="flex items-center justify-between rounded-lg border border-white/5 bg-white/5 px-3 py-2">
                  <div className="min-w-0">
                    ...
                    <p className="text-[9px] text-white/40">{[p.skill, p.location].filter(Boolean).join(" · ")}</p>
