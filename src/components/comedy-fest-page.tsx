...
import { LaughEmojiFace } from "@/components/laugh-emoji-face";
  ...
  return (
    <>
      <main className="arena-2030 relative min-h-screen overflow-hidden pb-56">
        <Arena2030Backdrop
          image="/comedy-fest-backdrop.png"
          imageOpacity="opacity-[0.4] sm:opacity-[0.46]"
          intensity="deep"
          photoFocus
        />
        ...
        <div className="relative z-10 px-4 pt-[11.5rem] sm:px-6 sm:pt-28 lg:px-8">
          <div className="mx-auto max-w-6xl">
            <Arena2030Header
              liveBadge="SUPER HILARIOUS"
              yearExtra={
                <p
                  className="a2030-micro text-[10px] font-bold uppercase text-[#00f5ff]/90 sm:text-xs"
                  aria-label="Laugh belly full"
                >
                  <LaughEmojiFace variant="grin" />{" "}
                  LAUGH BELLY FULL{" "}
                  <LaughEmojiFace variant="tears" />
                </p>
              }
              title="COMEDY FEST"
              titleJump
              description={
                <>
                  Ten young women from {comedyFestRegions} — holographic games, loud laughs, character play, and a live
                  room wired for the next decade.
                </>
              }
            />
            ...
            <ArenaLoungeFrostPanel className="a2030-holo-panel mt-8 rounded-[1.75rem] p-4 sm:p-5">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <p className="a2030-micro text-[10px] font-bold uppercase text-[#00f5ff] sm:text-xs">
                  Backdrop // 10 games // 10 players
                </p>
                <span className="a2030-pulse-ring inline-flex h-2 w-2 rounded-full bg-[#00f5ff]" />
              </div>
                ...
                {comedyFestGames.map((player) => (
                  ...
                  >
                    ...
                    <span className="text-[#a5b4fc]">
                      · {player.country === "United Kingdom" ? "UK" : player.country.split(" ")[0]}
                    </span>
                    <span className="text-[#00f5ff]">· {player.game}</span>
                  </span>
                ))}
              </div>
            ...
            {activePrompt ? (
              <ArenaLoungeFrostPanel className="a2030-transmission mt-6 rounded-[1.5rem] p-5 backdrop-blur-md">
                ...
                <p className="mt-3 text-sm leading-7 text-[#b8c9e8]">
            ...
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="a2030-micro text-[10px] font-bold uppercase text-[#ff2bd6] sm:text-xs">
                  Men&apos;s entry // drop low
                </p>
                <p className="text-sm text-[#9fb4d4]">Pick your play mode — holo prompts live below</p>
              </div>
              <button
                type="button"
                onClick={() => setPanelOpen((open) => !open)}
                className="a2030-micro rounded-lg border border-[#00f5ff]/30 bg-[#00f5ff]/8 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.12em] text-[#00f5ff] transition hover:border-[#ff2bd6]/45 hover:text-[#ff2bd6] sm:text-xs"
              >
                {panelOpen ? "Hide" : "Show"}
              </button>
            </div>
